...
from manopt_dr.predefined_func_generator import gen_cost_regularized_lda, gen_default_proj
...
for v in g.vertices():
    inst_id = g.vp['id'][v]
    stat = inst_stats[(inst_stats['InstitutionId'] == inst_id) *
...
node_labels[(g.vp['prestige_rank'].a < 0.25) *
            (g.vp['prestige_rank'].a >= 0.0)] = 0
...
def simplify_feat_names(names):
    replacer_list = [['all-', ''], ['variance', 'var'], ['maximum', 'max'],
                     ['minimum', 'min'], ['total_degree', 'degree'],
                     ['^', '-']]
    simplified_names = []
    for name in names:
...
for feat_idx in np.argsort(-np.abs(shap_values.values).mean(0))[:topk]:
    data['shap_values'][
        feat_names[feat_idx]] = shap_values.values[:, feat_idx].tolist()
