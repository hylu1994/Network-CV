...
def _prepare_data(args):
    ...
    if args['type'] == 'shap':
        ...
        labels = pd.DataFrame(data['nodes']).iloc[
            data['processed_node_indices']]['label'].tolist()
        ...
        for name in shap_values.keys():
            ...
            cs, order = to_scatterplot_info(xs,
                                            ys,
                                            labels,
                                            adjust_density=True)
        ...
        labels = np.array(
            pd.DataFrame(
                data['nodes']).iloc[data['processed_node_indices']]['label'])
        ...
        if len(args['selected_attr_names']) > 0:
            attr_indices = [
                attr_name_to_index[name]
                for name in args['selected_attr_names']
            ]
            ...
            if target_correlation == 'spearman':
                ...
                res = minimize(lambda w: -spearmanr(inputs @ w, lda_coords)[0],
                               best_weights,
                               method='COBYLA',
